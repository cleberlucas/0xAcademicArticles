...
contract RepositoryDoctoralThesis {
    struct PostID {
        address posterID;
        uint256 sequence;
    }
    ...
    struct StoreData {
        ...
        mapping(address posterID => mapping(uint256 sequence => AcademicRepositoryLibrary.DoctoralThesis)) Repository;
    }

    event DoctoralThesishRegistered(
        address indexed posterID,
        uint256 indexed sequence,
        string title
    );

    event DoctoralThesishUnRegistered(
        address indexed posterID,
        uint256 indexed sequence
    );
    ...
    function showDoctoralThesis(
        ...
        view
        returns (
            AcademicRepositoryLibrary.DoctoralThesis memory mastersThesish
        )
    {
    ...
    function registerDoctoralThesis(
        ...
        emit DoctoralThesishRegistered(posterID, sequence, mastersThesish.title);
    }
    ...
    function unregisterDoctoralThesis(uint256 sequence) public {
        ...
        emit DoctoralThesishUnRegistered(posterID, sequence);
    }  
}
